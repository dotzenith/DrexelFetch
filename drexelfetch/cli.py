...
@app.command()
def search(
    course: str = typer.Argument(..., help="The course name, ex. CS 164")
):
    """
    Search for information about a given course
    """
    course_info = fetch.info(course)
    print(f"{colors.red}{'Course ID:' : <14}{colors.end} {course_info['course number']}")
    print(f"{colors.blue}{'Course Name:' : <14}{colors.end} {course_info['course name']}")
    print(f"{colors.orange}{'Credits:' : <14}{colors.end} {course_info['credits']}")
    print(f"{colors.green}{'Prerequisites:' : <14}{colors.end} {course_info['prereqs']}")
